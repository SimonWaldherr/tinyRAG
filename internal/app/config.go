...
type appSettings struct {
	...
	// This is persisted to the configured settings file. It will not be returned verbatim
	...
	// configured settings file; the process reads TINYRAG_STORAGE_KEY at startup.
	...
	// Vector-cache controls for tinySQL v0.49.0's optional VEC_SEARCH result cache.
...
func loadOrCreateSettings(path string, defaults appSettings) (*settingsStore, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			// settings files, so a freshly created settings file never
...
func (ss *settingsStore) saveLocked() error {
	...
	b = append(b, '\n')
	if err := os.MkdirAll(filepath.Dir(ss.path), 0o700); err != nil {
