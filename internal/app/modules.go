...
func normalizeModules(mods []moduleConfig) []moduleConfig {
	...
	for i := range mods {
		...
		if mods[i].ID == "module-http-folder" {
			switch strings.TrimSpace(mods[i].Config["root_dir"]) {
			case "./module-files", "module-files":
				// Keep existing installations working after the runtime
				// directory moved below data/.
				mods[i].Config["root_dir"] = "data/module-files"
			}
		}
	}
