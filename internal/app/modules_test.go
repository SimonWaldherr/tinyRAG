...
func TestNormalizeModulesMigratesLegacyHTTPFolderPath(t *testing.T) {
	mods := normalizeModules([]moduleConfig{{
		ID:     "module-http-folder",
		Kind:   "http-folder",
		Config: map[string]string{"root_dir": "./module-files"},
	}})
	if got := mods[0].Config["root_dir"]; got != "data/module-files" {
		t.Fatalf("expected legacy HTTP folder path to migrate, got %q", got)
	}
}
