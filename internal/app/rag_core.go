...
func newRAG(lm lmProvider, k int, dbPath string, storageMode tinysql.StorageMode, maxMemMB int64) (*ragSystem, error) {
	...
	case tinysql.ModeWAL:
		if dbPath == "" {
			dbPath = "data/tinyrag.gob"
		}
		if err := ensureParentDirectory(dbPath); err != nil {
			return nil, fmt.Errorf("prepare wal database path: %w", err)
		}
		db, err = tinysql.OpenDB(tinysql.StorageConfig{
	...
	case tinysql.ModeDisk:
		if dbPath == "" {
			dbPath = "data/tinyrag.db"
		}
		if err := ensureParentDirectory(dbPath); err != nil {
			return nil, fmt.Errorf("prepare disk database path: %w", err)
		}
		db, err = tinysql.OpenDB(tinysql.StorageConfig{
	...
	case tinysql.ModeIndex:
		...
		if err := ensureParentDirectory(dbPath); err != nil {
			return nil, fmt.Errorf("prepare index database path: %w", err)
		}
	...
	case tinysql.ModeHybrid:
		...
		if err := ensureParentDirectory(dbPath); err != nil {
			return nil, fmt.Errorf("prepare hybrid database path: %w", err)
		}
		mem := maxMemMB * 1024 * 1024
		if mem <= 0 {
			mem = 256 * 1024 * 1024
